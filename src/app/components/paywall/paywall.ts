// src/app/components/paywall/paywall.ts
...
export class PaywallComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log('Paywall: Fetching products...');
    this.subscriptionService.getProducts().subscribe({
      next: (response) => {
        console.log('Paywall: Products loaded successfully', response);
      },
      error: (err) => {
   ...
   */
  handleJoinNow(): void {
    console.log('Paywall: Join Now button clicked!');
    this.errorMessage.set(null);
    ...
    this.auth.isAuthenticated$.pipe(
      take(1),
      tap(isAuthenticated => {
        console.log('Paywall: User authenticated:', isAuthenticated);
        if (!isAuthenticated) {
          // User needs to sign up first - redirect to Auth0
          console.log('Paywall: Redirecting to Auth0 login');
          ...
          console.log('Paywall: Creating checkout session for plan:', this.selectedPlan());
          this.createCheckoutSession();
        }
      })
    ).subscribe({
      error: (err) => {
        console.error('Paywall: Error checking auth status:', err);
